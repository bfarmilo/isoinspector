...
const getWebMData = tag => {
    ...
    const entryLookup = new Map([
        ['u', { description: 'unsigned integer', returnVal: (data, size) => ({ display: data.readUIntBE(0, size), hex:null }) }],
        ['i', { description: 'signed integer', returnVal: (data, size) => ({ display: data.readIntBE(0, size), hex:null }) }],
        ['f', { description: 'floating point number', returnVal: data => ({ display: data.readFloatBE(0), hex:null }) }],
        ['s', { description: 'ASCII string', returnVal: data => ({ display: data.toString(), hex:null }) }],
        ['8', { description: 'UTF-8 string', returnVal: data => ({ display: data.toString('utf8'), hex:null }) }],
        ['d', { description: 'timestamp', returnVal: data => { console.warn('timestamp', data); return { display: new Date(data), hex:null } } }],
